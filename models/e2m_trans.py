...
class CrossCondTransBase(nn.Module):
    ...
    def __init__(self, 
        ...
        self.cond_emb = nn.Identity() if clip_dim == embed_dim else nn.Linear(clip_dim, embed_dim)
        ...
        if num_local_layer > 0:
            self.word_emb = nn.Identity() if eeg_f_dim == embed_dim else nn.Linear(eeg_f_dim, embed_dim)
...
class EEG2Motion_Transformer(nn.Module):
    ...
    def __init__(self, 
        ...
        super().__init__()
        self.n_head = n_head
        self.trans_base = CrossCondTransBase(vqvae, num_vq, embed_dim, clip_dim,eeg_f_dim, block_size, num_layers, num_local_layer, n_head, drop_out_rate, fc_rate)
    ...
    def forward_function(self, idxs, eeg,subid, src_mask=None, att_txt=None):
        if src_mask is not None:
            src_mask = self.get_attn_mask(src_mask, att_txt)

        eeg_fea,eeg_fea_g = self.eegencoder(eeg,subid)
        # print(eeg_fea.shape)
        clip_feature = eeg_fea_g
        
        feat = self.trans_base(idxs, clip_feature, src_mask, eeg_fea)
        logits = self.trans_head(feat, src_mask)
        return logits,eeg_fea_g
    
    def sample(self, eeg,subid, m_length=None, if_test=False, rand_pos=True, CFG=-1, token_cond=None, max_steps = 10):
        max_length = 49
        ...
        scores = torch.ones(shape, dtype = torch.float32, device = eeg.device)
        ...
        if token_cond is not None:
            ...
            ids = torch.full(shape, mask_id, dtype = torch.long, device = eeg.device)
...
# forward_function(self, idxs, clip_feature, eeg, src_mask=None, att_txt=None)
            if CFG>0:
                out = self.forward(ids, eeg,subid, src_token_mask)
                logits_cond = out[0][:,1:]
                eeg_fea_g = out[1]
                out = self.forward(ids, torch.zeros_like(eeg,device=eeg.device),subid, src_token_mask)
                logits_un = out[0][:,1:]
                logits = logits_un + CFG*(logits_cond-logits_un)
            else:
                out = self.forward(ids, eeg,subid, src_token_mask)
                logits = out[0][:,1:]
                eeg_fea_g = out[1]
        ...
        if if_test:
            return ids,eeg_fea_g
        return ids,logits,eeg_fea_g
